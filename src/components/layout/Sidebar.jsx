...
export default function Sidebar({ mobileOpen, onClose }) {
  ...
  return (
    <aside className={`fixed left-0 top-16 h-[calc(100vh-4rem)] flex flex-col pt-6 pb-6 w-64 z-40 bg-surface border-r border-outline transition-transform duration-300 ${mobileOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0`}>
      ...
      <nav className="flex-1 space-y-1 overflow-y-auto px-2">
        {navGroups.map((group, gi) => (
          <div key={gi} className={gi > 0 ? 'mt-5' : ''}>
            {group.label && (
              <p className="px-4 mb-2 text-[10px] font-bold uppercase tracking-[0.15em] text-on-surface-variant/50">{group.label}</p>
            )}
            {group.items.map(item => (
              <NavLink
                ...
                onClick={onClose}
                className={({ isActive }) =>
                  `mx-2 flex items-center px-4 py-2.5 rounded-xl transition-all ${
                    isActive
